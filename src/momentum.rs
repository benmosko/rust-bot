...
impl Momentum {
    ...
    fn check_early_momentum(&self, spot_price: Decimal, elapsed_pct: f64) -> Option<(bool, String)> {
        if elapsed_pct > 0.5 {
            return None;
        }
        let move_pct = (spot_price - self.opening_price).abs() / self.opening_price;
        if move_pct >= self.config.mom_min_spot_move_pct {
            let up = spot_price > self.opening_price;
    ...
    fn check_late_confirmation(&self, spot_price: Decimal, time_remaining: i64) -> Option<(bool, String)> {
    ...
    async fn check_preemptive_cancel(&mut self) -> Result<()> {
        ...
        let current_up = spot.price > self.opening_price;
    ...
    pub async fn run(
        ...
        let order_size = self.market.minimum_order_size * dec!(10); // small size for momentum
        ...
        while !shutdown.is_cancelled() {
            ...
            let (is_up, side_str) = if time_remaining <= 30 {
                if let Some((up, s)) = self.check_late_confirmation(spot_price, time_remaining) {
                    (up, s)
                } else {
                    sleep(Duration::from_millis(200)).await;
                    continue;
                }
            } else if let Some((up, s)) = self.check_early_momentum(spot_price, elapsed_pct) {
                (up, s)
            } else {
                sleep(Duration::from_millis(500)).await;
                continue;
